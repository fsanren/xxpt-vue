<template>
  <div class="menu-wrapper">
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      ...
      >
        <el-tooltip :disabled="isCollapse" :content="item.children[0].meta.title" placement="right">
          <el-menu-item :index="item.path+'/'+item.children[0].path" :class="{'submenu-title-noDropdown':!isNest}">
            <svg-icon v-if="item.children[0].meta&&item.children[0].meta.icon" :icon-class="item.children[0].meta.icon" />
            <span v-if="item.children[0].meta&&item.children[0].meta.title" slot="title" class="itemdisplay">{{ item.children[0].meta.title }}</span>
          </el-menu-item>
        </el-tooltip>
      </router-link>
      ...
      <el-submenu v-else :key="item.name" :index="item.name||item.path">
        <template slot="title">
          ...
          <span v-if="item.meta&&item.meta.title" slot="title" class="itemdisplay">{{ item.meta.title }}</span>
        </template>
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          ...
          <router-link v-else :key="child.name" :to="item.path+'/'+child.path">
            <el-menu-item :index="item.path+'/'+child.path">
              <span style="font-size: 24px;margin: 0 4px 0 20px">·</span>
